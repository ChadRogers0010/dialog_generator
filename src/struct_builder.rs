...
pub fn build_query(path: impl Into<String> + Clone, lines_per_module: usize) {
    ...
    while let Some(line) = lines.next() {
        ...
        current_module.write(if_string.as_bytes());
    }
    current_module.write("\t}\n}".as_bytes());
    ...
    main_buf.write(mod_header.as_bytes());
    main_buf.write(print_statement.as_bytes());

    let modules_body = builder
        .module_handles
        .iter()
        .enumerate()
        .map(|(i, f)| format!("    mod_{i:#03}::mod_{i:#03}(predicates, response);"))
        .fold(String::new(), |f, s| f + "\n" + &s);

    main_buf.write(modules_body.as_bytes());
    main_buf.write("\n}".as_bytes());
}
...
fn new_module(dir_path: String, n: usize) -> (BufWriter<File>, String) {
    ...
    writer.write(mod_header.as_bytes());
    (writer, new_mod)
}

impl Builder {
    fn top_function_rs(&self) -> String {
        let declaration =
            String::from("pub fn query(predicates: &[i32], response: &mut Vec<&'static str>) {");
...
// fn mod_01(predicates: &[i32], response: &mut Vec<&'static str>) {}
// fn mod_02(predicates: &[i32], response: &mut Vec<&'static str>) {}
//
// pub fn query(predicates: &[i32], response: &mut Vec<&'static str>) {
//     mod_01(predicates, response);
//     mod_02(predicates, response);
// }
